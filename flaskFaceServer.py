...
# UPLOAD_FOLDER = './backend/public/user_images'  # Folder where images are stored
...
#         print("User %", user)
#         # Extract the file path of the user's photo
#         file_name = user[3]
#         file_path = os.path.join(UPLOAD_FOLDER, file_name)

#         if not os.path.exists(file_path):
#             return jsonify({"success": False, "message": "Stored image not found for user"}), 404

#         # Now process the uploaded image and compare it with the stored image
#         try:
...
# @app.route('/public/user_images/<filename>')
...
# UPLOAD_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), '/backend/public/user_images/')
...
#     image_name = os.path.basename(image_path)  # This will return only the file name with the extension
...
#         # Modify the file name if needed (e.g., to extract only the numeric part)
#         file_name = extract_image_name(file_name)  # Modify the name as needed
